...
protected:
  void closeEvent (QCloseEvent * event) override
  {
    ...
    for (int i = 0; i < tabWidget->count (); i++)
      {
	QTextEdit *textEdit =
	  qobject_cast < QTextEdit * >(tabWidget->widget (i));
  ...
  {
    QTextEdit *textEdit =
      qobject_cast < QTextEdit * >(tabWidget->widget (index));
    if (textEdit && textEdit->document ()->isModified ())
      {
	...
	QMessageBox::StandardButton button =
	  QMessageBox::warning (this, "Unsaved Changes",
				"There are unsaved changes. Do you want to save before closing?",
				QMessageBox::
				Save | QMessageBox::Discard | QMessageBox::
				Cancel, QMessageBox::Save);
